<template>
  <div class="grid grid-cols-12 project py-12 md:py-10">
    <div
      class="info col-span-12 lg:col-span-6 order-2 lg:order-1 flex flex-col justify-center px-4"
    >
      <h2
        class="text-5xl text-primary dark:text-white text-center lg:text-left lg:text-6xl font-bold"
      >
        {{ item?.name }}
      </h2>
      ...
      <ReadMore
        class="dark:text-gray-300 text-center lg:text-left md:text-lg capitalize mt-8"
    ...
    <div
      class="image col-span-12 lg:col-span-6 order-1 lg:order-2 cursor-pointer"
      @click="openGallery"
    >
      <NuxtImg
        height="400"
        width="750"
        class="h-[400px] w-full object-contain lg:object-cover"
        preload
        placeholder="https://placehold.co/750x400/161A29/546192?text=Loading..."
        :style="{ boxShadow: getBoxShadow(item.primaryColor) }"
        v-if="item?.screenShots[0]"
        :src="url(item?.screenShots[0])"
        :class="`rounded-3xl border-4 border-seconadry dark:border-text shadow-lg `"
      />
      <span
        class="bg-primary text-white p-2 rounded-xl relative left-4 bottom-12"
...
const url = (imageName: string) =>
  new URL(`../assets/projects/${imageName}`, import.meta.url).href;
